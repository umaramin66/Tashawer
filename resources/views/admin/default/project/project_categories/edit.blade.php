        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <form class="form-horizontal" action="{{ route('project-categories.update', $project_category->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group mb-3">
                        <label for="name">{{translate('Name Arabic')}}</label>
                        <input type="text" id="name" name="name_ar" placeholder="{{ translate('Category Name Arabic') }}" value="{{ $project_category->name_ar }}" class="form-control" required>
                    </div>
                    <div class="form-group mb-3">
                        <label for="name">{{translate('Description')}}</label>
                        <textarea type="text" id="des" name="des" placeholder="{{ translate('Category Name') }}" value="" class="form-control" required>{{ $project_category->des }}</textarea>
                    </div>
                     <div class="form-group mb-3">
                        <label for="name">{{translate('Description Arabic')}}</label>
                        <textarea type="text" id="des_ar" name="des_ar" placeholder="{{ translate('Category Description Arabic') }}" class="form-control" required>{{ $project_category->des_ar }}</textarea>
                    </div>
                    <div class="form-group mb-3">
